...
import validation from './validacionEdTPserv';
import Typography from '@mui/material/Typography';
import '../../login.css';
import ModalTPservicio from './modaltpservicio';
...
export default function DataTable() {
    ...
    const [editedTPservicio, setEditedEmployee] = useState(null);
    ...
    //Mostrar en la Tbla
    useEffect(() => {
        axios.get('http://localhost:8081/tpservicio/tipos-servicios')
            .then(response => {
    ...
    const handleSave = () => {
        ...
        if (noHayErrores) {
            axios.post('http://localhost:8081/tpservicio/actualizar-tpserv', {
                id_tiposer: editedTPservicio.id,
                datosTPservicio: editedFields
            })
                .then(res => {
                    const message = res.data.message;
                    if (message === "TP servicio actualizado exitosamente") {
                        setOpenNotification(true);
    ...
    //Tabla
    const columns = [
        { field: 'id', headerName: 'ID.Tp Servicio', width: 70, headerClassName: 'custom-header' },
        { field: 'nombre_tiposer', headerName: 'Nombre', width: 130, headerClassName: 'custom-header' },
        { field: 'valor_tiposer', headerName: 'Precio', width: 130, headerClassName: 'custom-header' },
        { field: 'estado_tiposer', headerName: 'Tipo Vehiculo', width: 130, headerClassName: 'custom-header' },
        { field: 'descripcion_tiposer', headerName: 'Descripcion', width: 130, headerClassName: 'custom-header' },
        {
            field: 'acciones', headerName: 'Acciones', width: 150, headerClassName: 'custom-header',
            renderCell: (params) => (
                <div>
                    {isEditing && editedTPservicio && editedTPservicio.id_tiposer === params.row.id ? (
                        <Button
                            variant="contained"
                            color="primary"
    ...
    return (
        <div className="table-consulta">
            ...
            <ModalTPservicio
                ...
                editedTPservicio={editedTPservicio}
